...
<section class="login-block">
  ...
  <div class="form-group">
    ...
    <input type="password" name="pass_admin" class="form-control" placeholder="">
  </div>
  <?php
    error_reporting(0);
    session_start();
  ...
  <?php
    ...
    if(isset($_POST['submit']))
    {
      ...
      if($id_admin=="" || $pass_admin=="")
      {
        echo "<p style='color:red'>Hãy điền đầy đủ thông tin</p>";   
      }
      else{
        $sql="select * from admin where id_admin='$id_admin' and pass_admin='$pass_admin'";
        $query=mysqli_query($connect,$sql);
        $num_rows=mysqli_num_rows($query);
        $row = mysqli_fetch_assoc($query);
        if ($num_rows!=0) {
          header("Location:trangchuadmin.php?ten_admin=".$row["ten_admin"]."");
        }
        else{
...
<div class="copy-text">VIBRANT <i class="fa fa-heart"></i> Việt Nam</div>
    ...
    <div class="carousel-item">
      ...
      <div class="carousel-caption d-none d-md-block">
        <div class="banner-text">
            <h2>VIBRANT</h2>
            <p>VIBRANT Group là một tập đoàn đa quốc gia có trụ sở chính được đặt tại Hàn Quốc và Nhật Bản. Lotte do doanh nhân người Hàn Quốc Shin Kyuk-ho thành lập lần đầu tiên vào tháng 6 năm 1948 tại Tokyo. Từ Tokyo, Lotte mở rộng sang Hàn Quốc với việc thành lập của Lotte Confectionery tại Seoul vào ngày 3 tháng 4 năm 1967 và cuối cùng phát triển trở thành tập đoàn kinh doanh đa ngành có quy mô lớn thứ 5 của Hàn Quốc.</p>
        </div>  
    </div>
    </div>
    <div class="carousel-item">
